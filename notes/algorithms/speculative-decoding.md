...
设大模型耗时 `T_L`，小模型耗时 `T_S`，接受率 `α`：

```
理想加速比（当 T_S → 0 时） ≈ k × α / (1 - α^k) 对于 k 个草稿 token

实际加速比 ≈ (1 + α + α² + ... + α^{k-1}) × T_L / (k × T_S + T_L)
            ≈ (1-α^k)/(1-α) / (1 + k × T_S/T_L)
```

**典型数据**（Llama 2 70B 为 target，Llama 2 7B 为 draft，k=4）：
- α ≈ 0.75（代码生成）, 0.55（通用文本）
- T_S/T_L ≈ 0.1（7B vs 70B 参数比≈1/10）
- 加速比：~2.5× 代码，~1.8× 通用文本

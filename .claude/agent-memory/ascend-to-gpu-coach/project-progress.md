...
description: 进度——A5 读码完成(2026-08-10)，B1 Triton Vector Add 已完成 GPU 验收、当前进入 MatMul；理论线主线 A DeepSeek-V3.2→V4。权威源 PATH.md
metadata:
  type: project
---
...
**How to apply:** profiling 数据让用户从 LeetGPU/AutoDL 实际 GPU 拿，别开本地 nvcc。不要按旧阶梯"不跳级"推进——用户定节奏，卡住帮看、要完整代码就给。Agent 代写的代码不算完成，必须用户自己重写并跑通。当前执行顺序是看原理 → LeetGPU 写题并通过 → 同步本地 → 真实 GPU → benchmark，纯 CUDA kernel 后置。相关：[[feedback-style]] [[code-ownership-clarification]]

# Week 2 — GEMM v0 Naive

> 目标：理解矩阵乘的 GPU 实现，写出 naive GEMM，定位瓶颈
> 时间：4-6 小时
> 前置：完成 Week 1，能写简单 kernel + 计时
> 平台：LeetGPU（`22_gemm` 题，FP16）或本地 FP32

📚 **本周涉及的底层知识**：
- [内存层级详解](../cuda-kernels/notes/memory-model.md) — coalescing、shared memory、arithmetic intensity
- [CUDA API 速查](../cuda-kernels/notes/cuda-cheatsheet.md) — 2D grid/block、计时
- [LeetGPU 题库](../cuda-kernels/notes/leetgpu-challenges.md) — `22_gemm` 题签名和约束
...
## Day 1：GEMM 是什么 + 为什么它是一切的基础（1h 阅读）
...
- [ ] 知道下一步优化方向：shared memory tiling（Week 3）
...
| CUDA API 速查（计时、grid/rlock） | [cuda-cheatsheet.md](../cuda-kernels/notes/cuda-cheatsheet.md) |
| LeetGPU GEMM 题完整规格 | [leetgpu-challenges.md](../cuda-kernels/notes/leetgpu-challenges.md) → `22_gemm` |
| Triton GEMM 的实现（之后对照） | [triton-cheatsheet.md](../triton-kernels/notes/triton-cheatsheet.md) |
| 优化阶梯详解 | [../leetgpu-roadmap.md](../leetgpu-roadmap.md) |
| GEMM 参考实现（先自己写！） | [../cuda-kernels/gemm/gemm.cu](../cuda-kernels/gemm/gemm.cu) |

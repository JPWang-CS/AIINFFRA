...
**重学复用规则：** 已完成实验、原始代码与对应测量证据继续有效；课程经过对应章节时直接复盘并可跳过，不清零、不重跑。包括早期 CUDA GEMM 的 FP32/FP16 LeetGPU 归档与 RTX4090 大 K 对照；只有验收证据缺失、环境发生变化需要复测，或用户明确继续优化时，才新增实验；新增教学内容不改变既有状态。

## 3. 当前确认事实

| 项目 | 状态 | 已确认事实 | 下一门槛 |
|---|---|---|---|
| CUDA Vector Add | `WIP（平台正确性已完成，归档缺口）` | Lesson 01 代码快照；HISTORY/weekly 记录 LeetGPU 跑通及 RTX4090 `696 GB/s / 0 error`；没有独立 `solutions/cuda` 原始`solve` | 若能取得平台原始提交，补本地归档；必要时补测量元数据，不要求重跑已有测量 |
...
| Triton Softmax | `LEETGPU_PASS` | LeetGPU #5，0.29 ms，47.0th percentile，原始版已归档 | RTX3090二维row-wise correctness、ms、effective GB/s |

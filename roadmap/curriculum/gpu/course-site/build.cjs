...
fs.rmSync(docsRoot, {recursive:true, force:true}); fs.mkdirSync(docsRoot, {recursive:true});
 for (const info of pages.values()) { const output = path.join(site, info.output); fs.mkdirSync(path.dirname(output), {recursive:true}); const body = render(info.text, info, 'doc'); const css = relativeUrl(info.output, 'styles.css') + '?v=' + styleRevision; const katexCss = relativeUrl(info.output, 'vendor/katex/katex.min.css'); const reader = relativeUrl(info.output, 'reader.js'); const back = relativeUrl(info.output, 'index.html'); const toc = info.headings.filter(h => h.level === 2 || h.level === 3).map(h => '<li><a href="#' + encodeURIComponent(h.id) + '">' + esc(h.text) + '</a></li>').join(''); fs.writeFileSync(output, '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' + esc(info.title) + ' · AIINFFRA 附属文档</title><link rel="stylesheet" href="' + katexCss + '"><link rel="stylesheet" href="' + css + '"></head><body class="doc-page"><header class="topbar"><a class="site-name" href="' + back + '?chapter=1">AIINFFRA</a><nav><a href="' + back + '?chapter=1">返回课程</a></nav></header><main class="content doc-content"><div class="breadcrumb">附属阅读文档 · ' + esc(repoRelative(info.file)) + '</div><nav class="chapter-outline" aria-label="附属文档目录"><p>目录</p><ol>' + toc + '</ol></nav><div class="markdown-body">' + body + '</div></main><script src="' + reader + '"></script></body></html>'); }

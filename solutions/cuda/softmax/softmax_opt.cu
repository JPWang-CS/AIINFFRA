// softmax 优化版 working copy —— 代码起点 = softmax_naive.cu（3-pass 跨 block，已跑通）
// 优化方向见 lessons/04-softmax.md Part 6：① online 融合 max+sum → ② warp shuffle reduce → ③ 去 host 归约
// 原始结构：Kernel 1 找全局 max → Kernel 2 算全局 sum → Kernel 3 normalize
//
// 测这一版：KERNEL=softmax_opt.cu ./run.sh
// 对比 naive：先 ./run.sh（naive），再 KERNEL=softmax_opt.cu ./run.sh（这版）
